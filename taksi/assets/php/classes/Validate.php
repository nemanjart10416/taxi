...
class Validate{
    ...
    public static function addReturnRide(){
        ...
        //return ride date cant be lowwer than ride date
        if(isset($_POST["returndate"]) && isset($_POST["date"])){

            $date = user_input($_POST["date"]);
            $returndate = user_input($_POST["returndate"]);

            if($date>$returndate){
                return danger("date for return ride can't be before original ride date");
            }
        }
        ...
        if($number_people<1 || $number_people>$enviroment->getMaxPeople()){
        ...
        if($number_suitcases<0 || $number_suitcases>$enviroment->getMaxSuitcases()){
            return danger("something is wrong here(for return ride)");
        }

        //baby seats begin
        if(!isset($_POST["returnbabySeat"])){
            return danger("you must have number of baby seats for return ride");
        }

        $number_babySeat = user_input($_POST["returnbabySeat"]);
        if((empty($number_babySeat) && $number_babySeat!=="0") || !is_numeric($number_babySeat)){
            return danger("you must have number of baby seats for return ride");
        }

        if($number_babySeat<0 || $number_babySeat>$enviroment->getMaxChildSeat()){
            return danger("something is wrong here(for return ride)");
        }
        //baby seat end
        ...
        if($number_childSeat<0 || $number_childSeat>$enviroment->getMaxChildSeat()){
            return danger("something is wrong here(for return ride)");
        }
        //child seat end

        //raised seats begin
        if(!isset($_POST["returnraisedSeat"])){
            return danger("you must have number of raised seats for return ride");
        }

        $number_raisedSeat = user_input($_POST["returnraisedSeat"]);
        if((empty($number_raisedSeat) && $number_raisedSeat!=="0") || !is_numeric($number_raisedSeat)){
            return danger("you must have number of raised seats for return ride");
        }

        if($number_raisedSeat<0 || $number_raisedSeat>$enviroment->getMaxChildSeat()){
            return danger("something is wrong here(for return ride)");
        }
        //child seat end
    ...
    public static function addNewRide(){
        ...
        //baby seats begin
        if(!isset($_POST["babySeat"])){
            return danger("you must have number of baby seats");
        }

        $number_babySeat = user_input($_POST["babySeat"]);
        if((empty($number_babySeat) && $number_babySeat!=="0") || !is_numeric($number_babySeat)){
            return danger("you must have number of baby seats");
        }
        ...
        //raised seats begin
        if(!isset($_POST["raisedSeat"])){
            return danger("you must have number of raised seats");
        }

        $number_raisedSeat = user_input($_POST["raisedSeat"]);
        if((empty($number_raisedSeat) && $number_raisedSeat!=="0") || !is_numeric($number_raisedSeat)){
            return danger("you must have number of raised seats");
        }
