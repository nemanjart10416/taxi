...
class Driver{
    ...
    public static function setRandomPassword($email){
        ...
        try{
            ...
            $query = "UPDATE driver SET driver_password=:hash,driver_salt=:salt WHERE driver_email=:email";
    ...
    public static function change_status($id,$status){
        try{
            $konekcija = konekcija_prepared();
            $query = "UPDATE driver SET driver_status=:status WHERE driver_id=:id";
            $sth = $konekcija->prepare($query);
    ...
    public static function login($user,$password) {
        ...
        if(password_verify($pass, $result["driver_password"])){
            ...
            header("location: /TsQWxwWDtP_admin/driver.php");
            die();
        }else{
            return danger("user not found");
        }
